object P11 extends Euler {
  ...
  def posns = {
    val h = for {
      j <- 0 until 20
      i <- 0 until 20-4
    } yield Seq((j,i),(j,i+1),(j,i+2),(j,i+3))
    val v = for {
      i <- 0 until 20
      j <- 0 until 20-4
    } yield Seq((j,i),(j+1,i),(j+2,i),(j+3,i))
    val dr = for {
      i <- 0 until 20-4
      j <- 0 until 20-4
    } yield Seq((i,j),(i+1,j+1),(i+2,j+2),(i+3,j+3))
    val dl = for {
      i <- 3 until 20
      j <- 0 until 20-4
    } yield Seq((i,j),(i-1,j+1),(i-2,j+2),(i-3,j+3))
  ...
  def prod(l: Seq[Int]) = l reduce (_*_)

  def flesh(v: Seq[Seq[(Int,Int)]]) = v map { l =>
    l map { case (i,j) => data(i)(j) }
  }

  def soln = {
    val (h, v, dr, dl) = posns
    val theFour = Seq(h,v,dr,dl) flatMap (flesh(_)) maxBy prod
    (theFour, prod(theFour))
  }
